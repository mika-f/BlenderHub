...
const fetchDailyReleases = async (platform: Platform): Promise<Version[]> => {
  ...
  try {
    ...
    const $ = cheerio.load(html);
    $("a", `section.platform-${platform}`).each((_, element) => {
      const elem = $(element);
...
const fetchExperimentalReleases = async (platform: Platform): Promise<Version[]> => {
  const versions: Version[] = [];

  try {
    const response = await got("https://builder.blender.org/download/branches/");
    const html = response.body;
    ...
    $("a", `section.platform-${platform}`).each((_, element) => {
      ...
      const version = $("span.name", elem)
        .text()
        .match(/Blender\s(?<version>\d+\.\d+(\.\d+)?\s.*)\sbranch/)!.groups!.version!;

      versions.push({ version, url: `https://builder.blender.org/download/branches${url}`, branch: "experimental" });
    });
  } catch (err) {
    // eslint-disable-next-line no-console
    console.error(err);
